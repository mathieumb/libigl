...
IGL_INLINE bool igl::list_to_sparse_matrix(const std::vector<std::vector<T > > & V,Eigen::PlainObjectBase<Derived>& M)
{
  ...
  // Resize output
  M.resize(m,n);
  M.setConstant(static_cast<T>(-1/*igl::NO_INDEX*/));
...
#ifdef IGL_STATIC_LIBRARY
template bool igl::list_to_sparse_matrix<double, Eigen::Matrix<double,-1,-1,0,-1,-1> >(std::vector<class std::vector<double, std::allocator<double> >, std::allocator<class std::vector<double,class std::allocator<double> > > > const &, Eigen::PlainObjectBase<Eigen::Matrix<double,-1,-1,0,-1,-1> > &);
#endif